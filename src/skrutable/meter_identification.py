...
class VerseTester(object):
	...
	def attempt_identification(self, Vrs):
		...
		if _DEBUG_TIMING: _t0 = _time.perf_counter()
		success_anuzwuB = self.test_as_anuzwuB(Vrs) # 1 if successful, 0 if not
		if _DEBUG_TIMING: _times['anuzwuB'] = _time.perf_counter() - _t0
		...
		if _DEBUG_TIMING: _times['samavftta_etc'] = _time.perf_counter() - _t0
		...
		if _DEBUG_TIMING: _times['jAti'] = _time.perf_counter() - _t0
		if _DEBUG_TIMING: _log_times(_times, Vrs.meter_label)
...
class MeterIdentifier(object):
				...
				for pos_cd in pos_iterators['cd']:
					...
					try:
						...
						if _DEBUG_TIMING:
							...
							_type_keys = ('anuzwuB', 'samavftta_etc', 'jAti')
	...
	def identify_meter(self, rw_str,
		...
		if _DEBUG_TIMING:
			...
			_type_keys = ('anuzwuB', 'samavftta_etc', 'jAti')
			_all_keys = _scan_keys + _type_keys + ('wiggle_scan', 'wiggle_type')
		...
		if _DEBUG_TIMING:
			_flush_keys = ('scan_clean', 'scan_translit', 'scan_syllabify', 'scan_weights', 'scan_morae_gana', 'anuzwuB', 'samavftta_etc', 'jAti')
